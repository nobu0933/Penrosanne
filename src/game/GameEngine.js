...
export class GameEngine {
  constructor({ playerCount=2, meeples=7, side=120, random=Math.random, fieldScoring=true, deckType="standard", allowArrowOverride=true, rules={} }={}) { this.random=random; this.deckType=deckType; this.rules={allowArrowOverride:rules.allowArrowOverride ?? allowArrowOverride,allowVerticalArrowPattern:rules.allowVerticalArrowPattern ?? true}; this._candidateGroups=null; const players=Array.from({length:playerCount},(_,index)=>createPlayer({id:`p${index+1}`,name:`Player ${index+1}`,meeples})); const deck=shuffle(createPrototypeDeck(random, deckType),random); this.tileOptions=[...deck.map((tile)=>structuredClone(tile)), ...deck.map((tile)=>mirrorTile(tile))]; this.state=createGameState({players,deck}); this.state.board=new Board(side); this.fillabilityCache=new Map(); this.fieldScoring=fieldScoring; this.start(); }
  start() { const start=this.state.deck.pop(); start.centerX=0;start.centerY=0;start.rotation=0;start.arrowPattern=this.rules.allowVerticalArrowPattern?null:"normal";this.state.board.add(start);this.fillabilityCache=createFillabilityCache(this.state.board,this.tileOptions,{allowVerticalArrowPattern:this.rules.allowVerticalArrowPattern});this.nextTurn(); }
  get activePlayer() { return this.state.players[this.state.turn]; }
  candidateGroups() { if(this.state.phase!=="placeTile") return {regular:[],relative:[],relativeBlocked:[],arrowOverride:[]}; if(this._candidateGroups) return this._candidateGroups; return this._candidateGroups=placementCandidateGroups(this.state.board,this.state.currentTile,{tileOptions:this.tileOptions,fillabilityCache:this.fillabilityCache,allowRelativePlacement:!this.activePlayer.relativePlacementUsed,allowArrowOverride:this.rules.allowArrowOverride,allowVerticalArrowPattern:this.rules.allowVerticalArrowPattern}); }
  ...
  placeTile(tile,{ignoreArrowMatching=false}={}) { if(this.state.phase!=="placeTile") throw new Error("不正なタイル配置です。"); const groups=this.candidateGroups(), regular=groups.regular.some((candidate)=>samePlacement(candidate,tile)), relative=groups.relative.some((candidate)=>samePlacement(candidate,tile)), override=groups.arrowOverride.some((candidate)=>samePlacement(candidate,tile)), permittedOverride=override&&ignoreArrowMatching&&this.rules.allowArrowOverride; if(!regular&&!relative&&!permittedOverride) throw new Error("絶対禁則配置のため配置できません。"); if(!isLegalPlacement(this.state.board,tile,{ignoreArrowMatching:permittedOverride,allowVerticalArrowPattern:this.rules.allowVerticalArrowPattern})) throw new Error("不正なタイル配置です。"); if(relative&&!regular)this.activePlayer.relativePlacementUsed=true; for(const [id, pattern] of Object.entries(tile.resolvedArrowPatterns || {})){const neighbor=this.state.board.getTile(id);if(neighbor)neighbor.arrowPattern=pattern;} delete tile.resolvedArrowPatterns; delete tile._ignoresArrowMatching; this.state.currentTile=structuredClone(tile);this.state.board.add(tile);this._candidateGroups=null;this.collectVertexChips(tile);this.fillabilityCache=updateFillabilityCache(this.state.board,tile,this.tileOptions,this.fillabilityCache,{allowVerticalArrowPattern:this.rules.allowVerticalArrowPattern});this.state.phase="placeMeeple";const options=this.meepleOptions();if(!this.activePlayer.meeples||!options.length){this.finishTurn();return [];}return options; }
