...
export class GameEngine {
  constructor({ playerCount=2, meeples=7, side=120, random=Math.random, fieldScoring=true, deckType="standard", rules={} }={}) { this.random=random; this.deckType=deckType; this.rules={allowVerticalMatchingPattern:rules.allowVerticalMatchingPattern ?? true}; this._candidateGroups=null; this._structuralFrontier=null; const players=Array.from({length:playerCount},(_,index)=>createPlayer({id:`p${index+1}`,name:`Player ${index+1}`,meeples})); const deck=shuffle(createPrototypeDeck(random, deckType),random); this.tileOptions=[...deck.map((tile)=>structuredClone(tile)), ...deck.map((tile)=>mirrorTile(tile))]; this.state=createGameState({players,deck}); this.state.board=new Board(side); this.fillabilityCache=new Map(); this.fieldScoring=fieldScoring; this.start(); }
  start() { const start=this.state.deck.pop(); start.centerX=0;start.centerY=0;start.rotation=0;start.matchingPattern=this.rules.allowVerticalMatchingPattern?null:"normal";start.matchingPatternOptions=this.rules.allowVerticalMatchingPattern?["normal","verticalInverse"]:["normal"];this.state.board.add(start);this.fillabilityCache=createFillabilityCache(this.state.board,this.tileOptions);this.nextTurn(); }
  ...
  candidateGroups() {
    ...
    const regular=placementCandidates(this.state.board,this.state.currentTile,{tileOptions:this.tileOptions,fillabilityCache:this.fillabilityCache,allowVerticalMatchingPattern:this.rules.allowVerticalMatchingPattern})
  ...
  placeTile(tile) { if(this.state.phase!=="placeTile") throw new Error("不正なタイル配置です。"); const regular=this.candidates().some((candidate)=>samePlacement(candidate,tile)); if(!regular) throw new Error("絶対禁則または形状マッチング規則のため配置できません。"); if(!isLegalPlacement(this.state.board,tile,{allowVerticalMatchingPattern:this.rules.allowVerticalMatchingPattern})) throw new Error("不正なタイル配置です。"); applyMatchingPatternDomains(this.state.board,tile); delete tile._inferenceStatus; this.state.currentTile=structuredClone(tile);this.state.board.add(tile);this._candidateGroups=null; this._structuralFrontier=null; this.collectVertexChips(tile);this.fillabilityCache=updateFillabilityCache(this.state.board,tile,this.tileOptions,this.fillabilityCache);this.state.phase="placeMeeple";const options=this.meepleOptions();if(!this.activePlayer.meeples||!options.length){this.finishTurn();return [];}return options; }
  ...
  nextTurn() { while(this.state.deck.length){this.state.currentTile=this.state.deck.pop();this.state.phase="placeTile";this._candidateGroups=null;if(this.candidates().length)return;this.state.discarded.push(this.state.currentTile);this.state.currentTile=null;this._candidateGroups=null;}this.finishGame(); }
